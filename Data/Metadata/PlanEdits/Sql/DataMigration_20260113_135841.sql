...
BEGIN
    ...
    BEGIN TRY
        BEGIN TRAN;
        SET IDENTITY_INSERT [ContactsUsageTbl] ON;
        ...
        SET IDENTITY_INSERT [ContactsUsageTbl] OFF;
        IF EXISTS (SELECT 1 FROM sys.identity_columns WHERE object_id = OBJECT_ID(N'ContactsUsageTbl'))
            DBCC CHECKIDENT (N'ContactsUsageTbl', RESEED);
        COMMIT;
    ...
    BEGIN CATCH
        IF XACT_STATE() <> 0 ROLLBACK;
        BEGIN TRY
            IF OBJECT_ID(N'ContactsUsageTbl') IS NOT NULL SET IDENTITY_INSERT [ContactsUsageTbl] OFF;
        END TRY BEGIN CATCH END CATCH
