//------------------------------------------------------------------------------
// TrackerSQL v3.x — CheckBrowser
// Shared infrastructure / utility: CheckBrowser.
//------------------------------------------------------------------------------
...
namespace TrackerSQL.Classes
{
    ...
    {
        public const string CONST_SESSION_RUNNINGONMOBILE = "RunningOnMoble";
        ...
        public bool fBrowserIsMobile()
        {
            if (HttpContext.Current.Request == null || HttpContext.Current.Request.ServerVariables["HTTP_USER_AGENT"] == null)
                return false;
            string input = HttpContext.Current.Request.ServerVariables["HTTP_USER_AGENT"].ToString();
            return input.Length >= 4 && (CheckBrowser.MobileCheck.IsMatch(input) || CheckBrowser.MobileVersionCheck.IsMatch(input.Substring(0, 4)));
        }
    }
}
