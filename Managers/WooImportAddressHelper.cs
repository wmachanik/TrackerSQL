...
namespace TrackerSQL.Managers
{
    ...
    public static class WooImportAddressHelper
    {
        ...
        /// <summary>
        /// True when addresses differ by more than <paramref name="minDifferenceRatio"/>
        /// (token Jaccard: 0 = identical, 1 = no shared tokens). Default 0.5 = &gt;50% different.
        /// </summary>
        public static bool AddressesDifferSignificantly(
            string storedBilling,
            string wooBilling,
            double minDifferenceRatio = 0.5)
        {
            string a = NormalizeCompare(storedBilling);
            string b = NormalizeCompare(wooBilling);
            ...
            var tokensA = TokenizeAddress(a);
            var tokensB = TokenizeAddress(b);
            ...
            int union = tokensA.Count + tokensB.Count - shared;
            if (union <= 0)
                return false;

            double jaccard = (double)shared / union;
            double difference = 1.0 - jaccard;
            return difference > minDifferenceRatio;
        }

        private static HashSet<string> TokenizeAddress(string normalized)
        {
            var set = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            if (string.IsNullOrWhiteSpace(normalized))
                return set;

            foreach (Match m in Regex.Matches(normalized.ToLowerInvariant(), @"[a-z0-9]+"))
            {
