    ...
    public class HolidayClosureProvider
    {
        ...
        private void GetHolidayDateWithCache()
        {
            lock (_sync)
            {
                ...
                try
                {
                    const string sql = @"SELECT HolidayClosureID, ClosureDate, DaysClosed, AppliesToPrep, AppliesToDelivery, ShiftStrategy, Description
                    ...
                    using (var rdr = db.ExecuteReader(sql))
                    {
                        while (rdr != null && rdr.Read())
                        {
                            ...
                            {
                                ID = rdr["HolidayClosureID"] == DBNull.Value ? 0 : Convert.ToInt32(rdr["HolidayClosureID"]),
                ...
                catch (Exception ex)
                {
                    AppLogger.WriteLog(SystemConstants.LogTypes.Database,
                        "HolidayClosureProvider: cache load failed: " + ex.Message);
                }
                           ...
                           out string error)
        {
            ...
            try
            {
                const string sql = @"INSERT INTO HolidayClosuresTbl
                    (ClosureDate, DaysClosed, AppliesToPrep, AppliesToDelivery, ShiftStrategy, Description)
                    VALUES (@ClosureDate, @DaysClosed, @AppliesToPrep, @AppliesToDelivery, @ShiftStrategy, @Description)";

                var parameters = new List<DBParameter>
                {
                    new DBParameter { ParamName = "@ClosureDate", DataValue = closureDate.Date, DataDbType = DbType.Date },
                    new DBParameter { ParamName = "@DaysClosed", DataValue = daysClosed < 1 ? 1 : daysClosed, DataDbType = DbType.Int32 },
                    new DBParameter { ParamName = "@AppliesToPrep", DataValue = appliesToPrep, DataDbType = DbType.Boolean },
                    new DBParameter { ParamName = "@AppliesToDelivery", DataValue = appliesToDelivery, DataDbType = DbType.Boolean },
                    new DBParameter { ParamName = "@ShiftStrategy", DataValue = shiftStrategy ?? "Forward", DataDbType = DbType.String },
                    new DBParameter { ParamName = "@Description", DataValue = string.IsNullOrWhiteSpace(description) ? (object)DBNull.Value : description, DataDbType = DbType.String }
                };
