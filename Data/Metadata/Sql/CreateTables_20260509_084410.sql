...
-- Metadata paths:
--   AccessSchema: C:\SRC\ASP.net\TrackerSQL\Migrations\MigrationRunner\Metadata\AccessSchema
--   PlanConstraints: C:\SRC\ASP.net\TrackerSQL\Migrations\MigrationRunner\Metadata\PlanEdits\PlanConstraints.json
-- Tables: 59, Columns: 523
-- Identity suppressed: Yes
-- Drop existing tables: Yes
-- Plan scan (source -> decision):
-- [ArchivedCustomersTbl] IGNORE (plan.Ignore=true)
-- [ArichivedOrdersTbl] Class=Copy Target=ArichivedOrdersTbl EmittedCols=11
...
-- [ClientUsageHistoryTbl] Class=Copy Target=ClientUsageHistoryTbl EmittedCols=13
-- [ClientUsageLinesTbl] Class=Copy Target=ContactsItemSvcSummaryTbl EmittedCols=7
-- [ClientUsageTbl] Class=Copy Target=ContactsItemsPredictedTbl EmittedCols=13
...
-- [LogTbl] Class=Copy Target=LogTbl EmittedCols=8
-- [MachineConditionsTbl] Class=Copy Target=EquipConditionsTbl EmittedCols=4
-- [NextRoastDateByCityTbl] Class=Copy Target=NextPreperationDateByAreasTbl EmittedCols=7
-- [OrderList] Class=Copy Target=OrderList EmittedCols=25
-- [OrdersTbl] Class=Normalize Header=OrdersTbl Lines=OrderLinesTbl Emitted(H/L)=12/5 Synth=[NewHeaderKey=OrderID, NewLineKey=OrderLineID, LinkFK=OrderID] 
-- [OrdersTbl_Apr26_2008] Class=Copy Target=OrdersTbl_Apr26_2008 EmittedCols=11
...
-- [tmpOrdersReplyTbl] Class=Copy Target=tmpOrdersReplyTbl EmittedCols=17
...
-- [UsageAveTbl] Class=Copy Target=UsageAveTbl EmittedCols=14
-- [UsageTblByDate] Class=Copy Target=UsageTblByDate EmittedCols=9
-- [UsedItemGroupTbl] Class=Copy Target=UsedItemGroupsTbl EmittedCols=7
-- [VisitLogTbl] Class=Copy Target=VisitLogTbl EmittedCols=10
-- [WeekDaysTbl] Class=Copy Target=WeekDaysTbl EmittedCols=2
-- [_ClientUsageTbl] Class=Copy Target=_ClientUsageTbl EmittedCols=11
SET ANSI_NULLS ON;
...
-- Drop FKs referencing or owned by [ArichivedOrdersTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[ArichivedOrdersTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[ArichivedOrdersTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[ArichivedOrdersTbl]', N'U') IS NOT NULL DROP TABLE [ArichivedOrdersTbl];
GO
    CREATE TABLE [ArichivedOrdersTbl]
    (
        [OrderID] INT NULL,
        [CustomerId] INT NULL,
...
GO
    ...
    (
        [AwayReasonID] INT NOT NULL,
        [ReasonDesc] NVARCHAR(100) NULL
        , CONSTRAINT [PK_AwayReasonTbl] PRIMARY KEY CLUSTERED ([AwayReasonID])
    );
GO

-- Drop FKs referencing or owned by [ClientUsageHistoryTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[ClientUsageHistoryTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[ClientUsageHistoryTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[ClientUsageHistoryTbl]', N'U') IS NOT NULL DROP TABLE [ClientUsageHistoryTbl];
GO
    CREATE TABLE [ClientUsageHistoryTbl]
    (
        [HistoryID] INT NULL,
        [CustomerId] INT NULL,
...
GO
    ...
    (
        [ContactsAccInfoID] INT NOT NULL,
        [ContactID] INT NULL,
...
-- Drop FKs referencing or owned by [ContactsItemsPredictedTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[ContactsItemsPredictedTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[ContactsItemsPredictedTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[ContactsItemsPredictedTbl]', N'U') IS NOT NULL DROP TABLE [ContactsItemsPredictedTbl];
GO
    ...
    (
        ...
        [ContactsItemsPredictedId] NVARCHAR(255) NOT NULL
        , CONSTRAINT [PK_ContactsItemsPredictedTbl] PRIMARY KEY CLUSTERED ([ContactsItemsPredictedId])
    );
GO

-- Drop FKs referencing or owned by [ContactsItemSvcSummaryTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[ContactsItemSvcSummaryTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[ContactsItemSvcSummaryTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[ContactsItemSvcSummaryTbl]', N'U') IS NOT NULL DROP TABLE [ContactsItemSvcSummaryTbl];
GO
    CREATE TABLE [ContactsItemSvcSummaryTbl]
    (
        [ContactsItemSvcSummaryId] INT NOT NULL,
        [ContactID] INT NULL,
        ...
        , CONSTRAINT [PK_ContactsItemSvcSummaryTbl] PRIMARY KEY CLUSTERED ([ContactsItemSvcSummaryId])
    );
GO
...
GO
    ...
    (
        [ContactItemUsageLineNo] INT NOT NULL,
        [ContactID] INT NULL,
...
GO
    ...
    (
        ...
        [PreferredAgentID] INT NULL,
        [SalesAgentID] INT NULL,
        [EquipentSN] NVARCHAR(50) NULL,
        [UsesFilter] BIT NULL,
...
GO
    ...
    (
        [ContactTrackedServiceItemsID] INT NOT NULL,
        [ContactTypeID] INT NULL,
        [ItemServiceTypeID] INT NULL,
        [Notes] NVARCHAR(MAX) NULL
...
GO
    ...
    (
        ...
        [ItemPrepDescription] NVARCHAR(50) NULL,
...
-- Drop FKs referencing or owned by [LogTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[LogTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[LogTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[LogTbl]', N'U') IS NOT NULL DROP TABLE [LogTbl];
GO
    CREATE TABLE [LogTbl]
    (
        [LogID] INT NULL,
...
-- Drop FKs referencing or owned by [NextPreperationDateByAreasTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[NextPreperationDateByAreasTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[NextPreperationDateByAreasTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[NextPreperationDateByAreasTbl]', N'U') IS NOT NULL DROP TABLE [NextPreperationDateByAreasTbl];
GO
    CREATE TABLE [NextPreperationDateByAreasTbl]
    (
        [NextPrepDayID] INT NOT NULL,
        [AreaID] INT NULL,
...
-- Drop FKs referencing or owned by [OrderList]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[OrderList]') OR fk.referenced_object_id = OBJECT_ID(N'[OrderList]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[OrderList]', N'U') IS NOT NULL DROP TABLE [OrderList];
GO
    CREATE TABLE [OrderList]
    (
        [ID] INT NULL,
        [Cleints] INT NULL,
        [Cln Tb] NVARCHAR(255) NULL,
        [Decal] NVARCHAR(255) NULL,
        [Filter] NVARCHAR(255) NULL,
        [Sidama] NVARCHAR(255) NULL,
        [Switch] NVARCHAR(255) NULL,
        [Terranova] NVARCHAR(255) NULL,
        [Alpino] NVARCHAR(255) NULL,
        [Yirga] NVARCHAR(255) NULL,
        [Marango] NVARCHAR(255) NULL,
        [Limu] NVARCHAR(255) NULL,
        [Harrar] NVARCHAR(255) NULL,
        [Mandheling] NVARCHAR(255) NULL,
        [Timana] NVARCHAR(255) NULL,
        [Marcala] NVARCHAR(255) NULL,
        [Antigua] NVARCHAR(255) NULL,
        [Decaf] NVARCHAR(255) NULL,
        [La Piram] NVARCHAR(255) NULL,
        [Asorgan] NVARCHAR(255) NULL,
        [Los Idol] NVARCHAR(255) NULL,
...
-- Drop FKs referencing or owned by [OrdersTbl_Apr26_2008]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[OrdersTbl_Apr26_2008]') OR fk.referenced_object_id = OBJECT_ID(N'[OrdersTbl_Apr26_2008]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[OrdersTbl_Apr26_2008]', N'U') IS NOT NULL DROP TABLE [OrdersTbl_Apr26_2008];
GO
    CREATE TABLE [OrdersTbl_Apr26_2008]
    (
        [OrderID] INT NULL,
        [CustomerId] INT NULL,
        [OrderDate] DATETIME NULL,
...
-- Drop FKs referencing or owned by [PredictedOrdersTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[PredictedOrdersTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[PredictedOrdersTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[PredictedOrdersTbl]', N'U') IS NOT NULL DROP TABLE [PredictedOrdersTbl];
GO
    CREATE TABLE [PredictedOrdersTbl]
    (
...
GO
    ...
    (
        [RecurringOrderItemID] INT NOT NULL,
...
-- Drop FKs referencing or owned by [tmpOrdersReplyTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[tmpOrdersReplyTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[tmpOrdersReplyTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[tmpOrdersReplyTbl]', N'U') IS NOT NULL DROP TABLE [tmpOrdersReplyTbl];
GO
    CREATE TABLE [tmpOrdersReplyTbl]
    (
        [ID] INT NULL,
        [CustomerId] INT NULL,
...
GO
    ...
    (
        [TotalCounterTrackerID] INT NOT NULL,
...
GO
    ...
    (
        [TrackerServiceItemID] INT NOT NULL,
        [ItemServiceTypeID] INT NULL,
...
-- Drop FKs referencing or owned by [UsageAveTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[UsageAveTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[UsageAveTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[UsageAveTbl]', N'U') IS NOT NULL DROP TABLE [UsageAveTbl];
GO
    CREATE TABLE [UsageAveTbl]
    (
        [ClientId] INT NULL,
        [CoffeeCycle] INT NULL,
        [CoffeeProvided] INT NULL,
        [PreferedCoffeeId] INT NULL,
        [LastOrderedDate] DATETIME NULL,
        [EstReorderDate] DATETIME NULL,
        [CupsPerDaysConsumed] INT NULL,
        [LastCleanDate] DATETIME NULL,
        [EstCleanDate] DATETIME NULL,
        [LastFilterDate] DATETIME NULL,
        [EstFilterDate] DATETIME NULL,
        [LastDescaleDate] DATETIME NULL,
        [EstDescaleDate] DATETIME NULL,
        [FitlerOnlyClient] BIT NULL
    );
GO

-- Drop FKs referencing or owned by [UsageTblByDate]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[UsageTblByDate]') OR fk.referenced_object_id = OBJECT_ID(N'[UsageTblByDate]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[UsageTblByDate]', N'U') IS NOT NULL DROP TABLE [UsageTblByDate];
GO
    CREATE TABLE [UsageTblByDate]
    (
        [UsaageID] INT NULL,
        [ClientID] INT NULL,
        [Date] DATETIME NULL,
        [CupCount] INT NULL,
        [CoffeeProvded] BIT NULL,
        [MachineCleaned] BIT NULL,
        [FilterProvided] BIT NULL,
        [MachineDescaled] BIT NULL,
        [CountCheckOnly] BIT NULL
    );
GO
...
-- Drop FKs referencing or owned by [VisitLogTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[VisitLogTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[VisitLogTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[VisitLogTbl]', N'U') IS NOT NULL DROP TABLE [VisitLogTbl];
GO
    CREATE TABLE [VisitLogTbl]
    (
        [ID] INT NULL,
        [Client] INT NULL,
        ...
        [Descaled] BIT NULL,
        [CoffeeQty] REAL NULL,
        [CoffeeTypeProvided] INT NULL,
        [InvRef] NVARCHAR(50) NULL,
        [Notes] NVARCHAR(MAX) NULL
    );
GO

-- Drop FKs referencing or owned by [WeekDaysTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[WeekDaysTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[WeekDaysTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[WeekDaysTbl]', N'U') IS NOT NULL DROP TABLE [WeekDaysTbl];
GO
    CREATE TABLE [WeekDaysTbl]
    (
...
-- Drop FKs referencing or owned by [_ClientUsageTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[_ClientUsageTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[_ClientUsageTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[_ClientUsageTbl]', N'U') IS NOT NULL DROP TABLE [_ClientUsageTbl];
GO
    CREATE TABLE [_ClientUsageTbl]
    (
        [CustomerId] INT NULL,
        [LastCupCount] INT NULL,
        [NextCoffeeBy] DATETIME NULL,
