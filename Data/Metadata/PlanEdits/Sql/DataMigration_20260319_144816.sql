...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            NULLIF(src.[ClientUsageLineNo], N'') AS [ContactsItemSvcSummaryId], NULLIF(src.[CustomerID], N'') AS [ContactID], CASE WHEN NULLIF(src.[Date], N'') IS NULL OR LEN(LTRIM(RTRIM(NULLIF(src.[Date], N'')))) = 0 THEN NULL ELSE COALESCE(TRY_CONVERT(datetime2(7), NULLIF(src.[Date], N''), 127), TRY_CONVERT(datetime2(7), NULLIF(src.[Date], N''), 126), TRY_CONVERT(datetime2(7), NULLIF(src.[Date], N''), 121), TRY_CONVERT(datetime2(7), NULLIF(src.[Date], N''), 103), TRY_CONVERT(datetime2(7), NULLIF(src.[Date], N''), 101), TRY_CONVERT(datetime2(7), NULLIF(src.[Date], N'')), CAST(NULL AS datetime2(7))) END AS [UsageDate], NULLIF(src.[CupCount], N'') AS [CupCount], NULLIF(src.[ServiceTypeId], N'') AS [ItemServiceTypeID], NULLIF(src.[Qty], N'') AS [Qty], NULLIF(src.[Notes], N'') AS [Notes]
